"""
Run: python -m analysis.plot_convergence
"""
...
FIG_DIR = os.path.join(HERE, "output", "figures", "convergence")
...
def load_history(dim: int) -> dict[str, pd.DataFrame]:
    ...
    for a in ALGOS:
        ...
        out[a] = pd.read_csv(path)
    return out


def aggregate_curve(sub: pd.DataFrame):
    piv = sub.pivot_table(index="n_evals", columns="seed", values="best_error")
    piv = piv.sort_index().ffill()
    n_evals = piv.index.to_numpy(dtype=float)
    med = np.maximum(piv.median(axis=1).to_numpy(), FLOOR)
    q25 = np.maximum(piv.quantile(0.25, axis=1).to_numpy(), FLOOR)
    q75 = np.maximum(piv.quantile(0.75, axis=1).to_numpy(), FLOOR)
    return n_evals, med, q25, q75
...
def plot_func(hist: dict, dim: int, func_id: int, out_dir: str) -> None:
    ...
    for a in ALGOS:
        ...
        x, med, q25, q75 = aggregate_curve(sub)
